...
class Ckan {
	...
	public function searchAll($string,$offset='0',$limit='20',$order=NULL,$org=NULL){
		$results = $this->transfer('api/search/package?all_fields=1&offset='.$offset.'&amp;limit='.$limit.'&order_by='.$order.'&q='.urlencode($string).'&agency='.urlencode($org));
		if (!$results->count){
	...
	public function searchResources($format,$offset='0',$limit='20'){
		$results = $this->transfer('api/search/resource?all_fields=1&format='.urlencode($format).'&amp;limit='.$limit);
		if (!$results->count){
			$results = array('error' => 'Search Error', 'msg' => 'No results were found.');
		}
		return $results;
	...
	public function getTagList(){
		$pkgs = $this->searchAll('',0,1000,'',$this->agency);
		$tags = array();
